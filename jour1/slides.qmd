...
1. environ `r dollar(100 * coef(modele_prix)[["gr_liv_area"]], accuracy = 100)` pour 100 pieds carrés supplémentaires;
2. environ `r dollar(coef(modele_prix)[["overall_qual"]], accuracy = 100)` pour un point de qualité;
3. VIF entre `r round(min(check_collinearity(modele_prix)$VIF), 1)` et `r round(max(check_collinearity(modele_prix)$VIF), 1)`;
4. prédiction centrale près de `r dollar(predict(modele_prix, propriete_type)[[1]], accuracy = 1000)`;
