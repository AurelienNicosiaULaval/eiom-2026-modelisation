...
## Choisir peu de variables, mais les choisir pour une raison

| Variable | Rôle possible | Unité |
|---|---|---|
| `gr_liv_area` | taille | pied carré |
| `overall_qual` | qualité globale | score 1 à 10 |
| `year_built` | ancienneté approximative | année |
| `garage_cars` | équipement | nombre de places |
| `neighborhood` | contexte spatial | catégorie |

Raisons défendables: connaissance du problème, preuve graphique, contrôle d'un facteur et interprétabilité.

::: {.notes}
4 minutes. Faire choisir individuellement quatre variables. Demander ensuite si une liste de 80 variables serait automatiquement meilleure. Réponse: non, surtout sans objectif ni validation.
:::
...
## Numérique, ordinale ou catégorielle?

`overall_qual` possède dix niveaux ordonnés. Nous la codons de 1 à 10, ce qui suppose un écart régulier entre deux niveaux successifs.

`neighborhood` est nominale. R créerait une comparaison avec un quartier de référence pour chaque niveau.

Questions à poser:

1. L'ordre existe-t-il?
2. L'écart d'un niveau a-t-il un sens constant?
3. Une catégorie de référence est-elle claire?

::: {.notes}
4 minutes. Faire donner un exemple de variable nominale et ordinale. Nommer explicitement l'hypothèse imposée par le codage 1 à 10.
:::
...
## L'ordonnée à l'origine n'est pas toujours une histoire

Le modèle calcule un prix lorsque:
...
Cette combinaison est hors du domaine des données.

<div class="takeaway">Un coefficient peut être mathématiquement nécessaire sans être substantiellement interprétable.</div>

::: {.notes}
2 minutes. Montrer pourquoi l'ordonnée à l'origine est négative sans parler de modèle « faux ». Mentionner le centrage comme prolongement.
:::
...
3. une phrase choisissant le bon intervalle.

::: {.notes}
25 minutes. À 12 minutes, vérifier les unités. À 20 minutes, demander de corriger tout langage causal. Garder cinq minutes pour la comparaison des intervalles.
:::
...
1. environ 5 700 dollars pour 100 pieds carrés supplémentaires;
2. environ 24 000 dollars pour un point de qualité;
3. VIF entre 1,6 et 2,4;
4. prédiction centrale près de 193 000 dollars;
5. intervalle individuel beaucoup plus large que l'intervalle moyen.

<div class="takeaway">Une prédiction sans intervalle donne une précision que le modèle ne possède pas.</div>

::: {.notes}
15 minutes. Faire lire deux interprétations. Corriger unités, conditionnement et causalité. Faire expliquer la différence entre les intervalles par une équipe.
:::
...
## Quatre questions diagnostiques

1. La moyenne est-elle correctement structurée?
2. La variance des résidus est-elle à peu près stable?
3. Les résidus extrêmes dominent-ils l'inférence?
4. Quelques ventes changent-elles fortement les coefficients?

À cela s'ajoutent la dépendance entre observations et la pertinence de la population visée.

::: {.notes}
3 minutes. Associer chaque question à un graphique ou une mesure: résidus ajustés, QQ, Cook et contexte de collecte.
:::
...
bind_rows(
  ...
  tidy(modele_sans_max) |> mutate(Analyse = "Sans la vente 1499")
) |>
  filter(term != "(Intercept)") |>
...
1. un choix argumenté entre prix brut et logarithmique;
2. un verdict sur l'observation influente;
3. une fiche modèle de cinq phrases.

::: {.notes}
20 minutes. À 8 minutes, demander un choix d'échelle provisoire. À 15 minutes, faire commencer la fiche même si l'analyse n'est pas terminée.
:::
...
6. La conclusion dépasse-t-elle les données?
