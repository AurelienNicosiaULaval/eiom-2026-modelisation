...
::: {.notes}
4 minutes. Préciser que sept jours est une définition pédagogique, pas une norme officielle de service.
...
::: {.notes}
4 minutes. Demander si `dernier_statut`, `date_dernier_statut` ou `delai_dernier_statut_jours` peuvent être utilisés comme prédicteurs. Réponse: non.
:::
...
::: {.notes}
4 minutes. Demander ce que ce changement peut modifier: une référence naïve, la calibration et parfois les mesures de classement.
:::
...
::: {.notes}
3 minutes. Faire classer oralement les variables permises et interdites.
:::
...
::: {.notes}
3 minutes. Rappeler que la régression logistique sert ici de référence sérieuse, pas de modèle faible.
:::
...
::: {.notes}
3 minutes. Demander quel critère domine si une règle doit être expliquée ligne par ligne.
:::
...
::: {.notes}
3 minutes. Faire le lien avec la journée 2. Aucun calcul de coefficient n'est nécessaire aujourd'hui.
:::
...
::: {.notes}
4 minutes. Faire lire le chemin d'une demande fictive. Expliquer qu'un terminal donne une fréquence estimée, pas une certitude.
:::
...
::: {.notes}
3 minutes. Dire que l'impureté quantifie le mélange des classes, sans dériver le critère de Gini.
...
::: {.notes}
3 minutes. Demander la classe prédite à un seuil de 0,50 puis à 0,70. Réponses: non terminée, puis terminée.
:::
...
::: {.notes}
4 minutes. Faire prédire quelle erreur diminue presque toujours avec la profondeur. Réponse: l'erreur d'entraînement.
:::
...
::: {.notes}
3 minutes. Montrer les valeurs fixées: profondeur 10, minimum 30 et pénalité 0,001. Elles ne sont pas optimisées sur le test.
:::
...
::: {.notes}
4 minutes. Demander pourquoi des arbres identiques n'apporteraient rien. La diversité permet à l'agrégation de réduire une partie de la variance [@breiman2001random].
:::
...
::: {.notes}
3 minutes. Distinguer la taille de l'ensemble et la complexité de chaque arbre.
...
::: {.notes}
4 minutes. Faire corriger la phrase: « l'activité est la cause principale du délai ».
:::
...
::: {.notes}
3 minutes. Préciser qu'une forêt peut être plus stable qu'un arbre tout en étant plus flexible, grâce à l'agrégation.
:::
...
::: {.notes}
4 minutes. Demander quelles décisions sont prises avant le test: variables, prétraitement, candidats, paramètres, mesures et mandat.
:::
...
::: {.notes}
3 minutes. Relier moyenne et stabilité: deux questions complémentaires, pas deux versions de la même mesure.
:::
...
::: {.notes}
4 minutes. Faire vérifier que chaque validation est strictement postérieure à son apprentissage.
...
::: {.notes}
4 minutes. Demander quel protocole imite l'usage annoncé. Réponse: les fenêtres temporelles.
:::
...
::: {.notes}
3 minutes. Expliquer que changer les données en même temps que l'algorithme empêche d'attribuer le gain au modèle.
:::
...
::: {.notes}
4 minutes. Demander pourquoi on ne regroupe pas les catégories avec tout le fichier avant les plis. Réponse: cela utiliserait de l'information des validations.
...
::: {.notes}
4 minutes. Faire nommer l'événement positif: non terminée dans les sept jours.
:::
...
::: {.notes}
4 minutes. Comparer mentalement une erreur à 0,55 et une erreur à 0,95 lorsque y vaut 0. Le score moyen est minimisé à 0.
...
::: {.notes}
7 minutes. Faire corriger une équipe par une autre. Ne pas encore montrer les résultats de validation.
:::
...
::: {.notes}
5 minutes. Faire lire le sens de chaque colonne. Refuser « 72,6 % des demandes sont bien classées » pour l'aire ROC.
:::
...
::: {.notes}
4 minutes. Faire identifier le candidat le plus variable. Réponse: la forêt, avec un écart-type d'environ 0,016 contre 0,007 à 0,008.
:::
...
::: {.notes}
4 minutes. Demander quel modèle une organisation très aversive à l'instabilité pourrait retenir.
:::
...
::: {.notes}
3 minutes. Distribuer un mandat par équipe. Une décision acceptable doit citer au moins deux mesures, la stabilité et un coût.
:::
...
::: {.notes}
8 minutes. Faire entendre deux recommandations différentes. Montrer qu'elles peuvent être cohérentes si les mandats diffèrent.
:::
...
::: {.notes}
5 minutes. Comparer les résultats aux décisions écrites. La forêt reste première sur les trois mesures du test.
:::
...
::: {.notes}
4 minutes. Faire décrire les changements avant d'expliquer: baisse des aires pour logistique et arbre, forêt similaire, Brier plus faible dans le test en partie parce que la fréquence change.
:::
...
::: {.notes}
3 minutes. Demander ce qui arriverait si on réglait maintenant `mtry` sur octobre à décembre. Réponse: ce test ne pourrait plus confirmer le modèle réglé.
:::
...
::: {.notes}
4 minutes. Faire pointer la diagonale idéale. Dire que la forêt tend à surestimer les groupes de probabilité élevée dans ce test.
:::
...
::: {.notes}
4 minutes. Faire distinguer trois affirmations: utilisée par le modèle, associée à la cible, cause du délai. Seule la première est directement soutenue ici.
:::
...
::: {.notes}
4 minutes. Demander un exemple de seuil de retrait et exiger une durée, une mesure et une action.
:::
...
::: {.notes}
5 minutes. Faire lire une fiche par une équipe et demander au groupe quelle rubrique manque ou reste imprécise.
:::
...
::: {.notes}
4 minutes. Faire répondre une équipe à chacune des questions avec un élément de sa fiche.
:::
...
::: {.notes}
3 minutes. Billet de sortie. Exemple: « Si le Brier mensuel dépasse 0,24 pendant deux mois consécutifs, suspendre les scores et réexaminer les données et la calibration. » Préciser que 0,24 est un seuil pédagogique à justifier, pas une norme universelle.
:::
